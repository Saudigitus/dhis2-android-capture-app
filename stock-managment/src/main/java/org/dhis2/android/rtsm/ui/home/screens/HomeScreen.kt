...
fun checkVisibility(viewModel: HomeViewModel, manageStockViewModel: ManageStockViewModel): Boolean {
    return if ((
        ...
        return viewModel.hasFacilitySelected.collectAsState().value && manageStockViewModel.sizeTableData.collectAsState().value > 0
    } else if ((
        viewModel.toolbarTitle.collectAsState().value.name ==
        ...
        (
            ...
            viewModel.hasDestinationSelected.collectAsState().value && manageStockViewModel.sizeTableData.collectAsState().value > 0
        )
}
